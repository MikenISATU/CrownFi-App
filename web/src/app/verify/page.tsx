...
export default function VerifyPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.25fr)_minmax(280px,0.75fr)]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              CrownFi uses your connected wallet to locate the vote receipt. No
              receipt ID needs to be copied manually.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {!fan && (
                <p className="text-sm text-gold-soft/50">
                  Connect Freighter to identify your vote.
                </p>
              )}
            </div>
            ...
            {address && (
              <div className="rounded-2xl border border-line bg-black/25 px-4 py-3">
                <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-gold-soft/40">
                  Wallet used for lookup
                </div>
                <div className="mono mt-1 text-sm text-gold-soft">
                  {short(address, 10)}
                </div>
              </div>
            )}
            ...
            {err && (
              <div
                role="alert"
                className="flex items-start gap-3 rounded-2xl border border-ruby/30 bg-ruby/10 px-4 py-3 text-sm text-ruby"
              >
            ...
            <ol className="space-y-4 text-sm text-gold-soft/60">
              {[
                ...
                <li key={step} className="flex gap-3">
                  <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full border border-gold/25 bg-gold/10 text-xs font-semibold text-gold">
                    {index + 1}
                  </span>
      ...
      {result && (
        <Card
          className={result.verified ? "border-emerald/30" : "border-ruby/30"}
        >
          <CardHeader>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="flex items-start gap-3">
                <span
                  className={`grid h-11 w-11 shrink-0 place-items-center rounded-full ${result.verified ? "bg-emerald/15 text-emerald" : "bg-ruby/15 text-ruby"}`}
                >
                  {result.verified ? (
              ...
              <Badge tone={result.verified ? "success" : "danger"}>
                {result.mock ? "Local demo proof" : "Stellar checkpoint"}
              </Badge>
            </div>
          </CardHeader>
          <CardContent>
            <details className="group rounded-2xl border border-line bg-black/25">
              <summary className="cursor-pointer list-none px-4 py-3 text-sm font-semibold text-gold-soft transition hover:text-white">
                ...
                <span className="float-right text-gold-soft/40 transition group-open:rotate-180">
                  ⌄
                </span>
              </summary>
              <dl className="space-y-3 border-t border-line px-4 py-4 text-sm">
                {[
                  ...
                  [
                    "Leaf index",
                    result.index == null ? undefined : String(result.index),
                  ],
                  ["Proof steps", String(result.proof?.length ?? 0)],
                ].map(([label, value]) => (
                  <div
                    key={label}
                    className="grid gap-1 border-b border-line/60 pb-3 last:border-0 last:pb-0 sm:grid-cols-[150px_1fr]"
                  >
                    <dt className="text-gold-soft/45">{label}</dt>
                    <dd className="mono break-all text-xs text-gold-soft">
                      {value
                        ? label === "Leaf index" || label === "Proof steps"
                          ? value
                          : short(value, 18)
                        : "Not available"}
                    </dd>
