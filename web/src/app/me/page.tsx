...
export default function MePage() {
  ...
  if (!ready) {
    return (
      <div className="flex min-h-72 items-center justify-center text-gold-soft/50">
        <LoaderCircle className="mr-2 animate-spin" size={20} /> Loading your
        CrownFi profile…
      </div>
    );
  }
  ...
  const votes: DashboardRow[] = (data?.votes ?? []).map((vote: any) => ({
    main: vote.contestant,
    sub: vote.round,
    tag: vote.status,
  }));
  ...
  const collectibles: DashboardRow[] = (data?.collectibles ?? []).map(
    (collectible: any) => ({
      main: collectible.title,
      sub: `${collectible.priceUsdc} USDC`,
      tag: collectible.tokenId
        ? `Token ${short(collectible.tokenId, 5)}`
        : "Collected",
    }),
  );
      ...
      <div className="grid gap-4 md:grid-cols-[0.65fr_1.35fr]">
        <Card className="border-gold/25 bg-gold/5">
          <CardContent className="pt-5">
            ...
            <div className="mt-3 font-display text-5xl font-semibold text-gold">
              {fan.points ?? 0}
            </div>
            <p className="mt-2 text-sm text-gold-soft/45">
              Earn points by participating in supported CrownFi experiences.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <Wallet className="text-gold" size={19} />
              <CardTitle>Connected Stellar wallet</CardTitle>
            </div>
            <CardDescription>
              This public address identifies your CrownFi account. CrownFi never
              asks for your seed phrase.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="mono break-all rounded-2xl border border-line bg-black/25 px-4 py-3 text-sm text-gold-soft">
              {address}
            </div>
          </CardContent>
        </Card>
        ...
        {rows.length === 0 ? (
          ...
          <div className="space-y-2">
            {rows.map((row, index) => (
              <div
                key={`${row.main}-${index}`}
                className="rounded-2xl border border-line bg-black/25 px-4 py-3"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 truncate text-xs text-gold-soft/40">
                      {row.sub}
                    </div>
                  </div>
                  {row.tag && (
